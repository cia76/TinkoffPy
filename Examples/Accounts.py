...
if __name__ == '__main__':  # Точка входа при запуске этого скрипта
    ...
    for account_id in Config.AccountIds:  # Пробегаемся по всем счетам
        logger.info(f'Учетная запись {account_id}')
        request = PortfolioRequest(account_id=account_id, currency=tp_provider.currency)
        ...
        request = GetOrdersRequest(account_id=account_id)
        ...
        request = GetStopOrdersRequest(account_id=account_id)
